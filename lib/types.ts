...
export interface AcademicProgram {
  ...
  code: string;
  /** Thai display name (academic_program.label_th). */
  label: string;
  sortOrder: number;
}
...
export interface Curriculum {
  ...
  code: string;
  /** Owning academic program — drives the cascade in the entry form. */
  programCode: string;
  /** Thai display name (curriculum.label_th). */
  label: string;
  sortOrder: number;
}

/** Read-only reference catalog behind the program/curriculum pickers and the
 *  KPI library's batch metric presets. Served by /api/academic-catalog. */
...
export type DataSourceColumnType =
  ...
  | "program" // stores an academic_program code ("PH")
  | "curriculum"; // stores a curriculum code ("PHB")

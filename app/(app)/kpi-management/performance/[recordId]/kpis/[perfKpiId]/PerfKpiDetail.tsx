...
/**
 * The full editable KPI detail. Hosted both by the standalone route
 * (`variant="page"`, with a PageHeader + breadcrumbs) and by the slide-in
 * drawer on the performance record page (`variant="drawer"`, header supplied by
 * the Drawer chrome). Ids arrive as props so both surfaces can drive it.
 */
export function PerfKpiDetailBody({
  recordId,
  perfKpiId,
  variant,
}: {
                ...
                kpi.hasChildren ? (
                  <div className="border-t border-hairline">
                    ...
                    {metrics.length === 0 ? (
                      ...
                      <Table>
                        <colgroup>
                          {/* Metric no. shrinks to its badge; Metrics claims the
                              freed width. Remaining columns stay auto. */}
                          <col style={{ width: "1%" }} />
                          <col style={{ width: "45%" }} />
                          <col />
                          <col />
                        ...
                        <thead>
                          <tr>
                            <Th className="whitespace-nowrap">Metric no.</Th>
                            <Th>Metrics</Th>
...
/**
 * The same editable detail, slid in from the right instead of a full-page
 * navigation. Mirrors DashboardKpiDrawer's shape; the header is built from a
 * lightweight (cached) usePerfKpi call while the body owns the real work.
 */
export function PerfKpiDetailDrawer({
  recordId,
  perfKpiId,
  open,
  onClose,
  ...
  const typeName =
    ...
    (kpiTypesQ.data?.find((t) => t.id === kpi.kpiType)?.kpiTypeName ??
      KPI_TYPES.find((t) => t.id === kpi.kpiType)?.label ??
      kpi.kpiType);
  const recordUrl = `/kpi-management/performance/${recordId}/kpis/${perfKpiId}`;
  ...
  return (
    ...
    >
      <RequirePermission action="submit_metrics">
        <PerfKpiDetailBody recordId={recordId} perfKpiId={perfKpiId} variant="drawer" />
      </RequirePermission>
    </Drawer>
  );
}

...
const COLUMN_MAP: Record<string, string> = {
  ...
  routineCategoryId: "routine_category_id",
  kpiType: "kpi_type",
...
export async function DELETE(
  ...
  try {
    ...
    try {
      ...
      if (await activeRecordsHaveKpiProgress(conn, kpiId)) {
        await conn.rollback();
        return NextResponse.json(
          {
            error:
              "Can't delete: an active performance record already has recorded progress for this KPI (or its sub-KPIs). Close or adjust that record first.",
          },
          { status: 409 },
        );
      }

      const setId = await setIdForKpi(conn, kpiId);
      const [result] = await conn.query<ResultSetHeader>(
      ...
      // Remove it from active records too.
    ...
    return NextResponse.json({ id: kpiId });
  } catch (err) {
    return NextResponse.json(
